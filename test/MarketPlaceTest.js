...
contract('Marketplace', ([deployer, seller, buyer]) => {
  ...
  describe('products', async () => {
    ...
    it('lists products', async () => {
      const product = await marketplace.product(productCount)
    ...
    it('sells products', async () => {
      ...
      assert.equal(event.id.toNumber(), productCount.toNumber(), 'id is correct')
      assert.equal(event.name, 'iPhone X', 'name is correct')
      assert.equal(event.pr      ice, '1000000000000000000', 'price is correct')
      ...
      await marketplace.buyProduct(99, { from: buyer, value: web3.utils.toWei('1', 'Ether')}).should.be.rejected;      // FAILURE: Buyer tries to buy without enough ether
